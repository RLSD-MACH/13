...
class ContactsInherit(models.Model):
    ...
    def control_vat_on_vies(self, silent=False, write = False, vat=False):
        ...
        for partner_id in self.ids:
            ...
            if not vat and not write:

                vat = partner.vat
            
            if vat and partner.parent_id.id == False:
                ...
                if len(vat) > 5:
                    ...
                    request = request.replace("COUNTRY", self.encodeXMLText(countryCode))
                    request = request.replace("VATNUMBER", self.encodeXMLText(cvrNumber))
                    request = request.replace("Asker_countryCode", self.encodeXMLText(Asker_countryCode))
                    request = request.replace("Asker_CVR", self.encodeXMLText(Asker_CVR))
                    request = request.replace("partner_city", self.encodeXMLText(partner.city if partner.city else "" ))
                    request = request.replace("partner_zip", self.encodeXMLText(partner.zip if partner.zip else ""))
                    request = request.replace("partner_street", self.encodeXMLText((partner.street if partner.street else "") + ("\n" + partner.street2 if partner.street2 else "") ))
                    request = request.replace("partner_name", self.encodeXMLText(partner.name))
                    request = request.replace("partner_CompanyType", self.encodeXMLText(partner.name))
    ...
    def _write_control_vies(self, data):
        ...
        if 'vat' in data and 'last_vies_message_id' not in data:
            ...
            if auto:
                ...
                data['last_vies_message_id'] = self.control_vat_on_vies(silent=True, write = True, vat=data['vat'])
